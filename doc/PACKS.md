...
* `bool :AddClasses(jsonfilename)`: load classes from json, available since 0.34.1
...
* `bool :OpenLink(url, description)`: prompts user to open web page using default browser. Returns `true` if URL validation passes, `false` otherwise. Only available in PopTracker, since 0.35.4
* `bool .BulkUpdate`: can be set to true from Lua to pause running logic rules. NOTE: Since failing to set `BulkUpdate` back to `false` will stop PopTracker from updating, make sure your code between setting `BulkUpdate` is error resistant (for example by using `pcall`).
